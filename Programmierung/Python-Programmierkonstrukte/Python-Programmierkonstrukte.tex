...
Das Zeichen \textbf{\texttt{=}} ist der \textbf{Zuweisungsoperator}. Er weist der Variable auf der linken Seite den Wert auf der rechten Seite zu. Der Zuweisungsoperator ist nicht zu verwechseln mit dem \textbf{Vergleichsoperator} \textbf{\texttt{==}}, der zwei Werte vergleicht und einen booleschen Wert (\textbf{\texttt{True}} oder \textbf{\texttt{False}}) zurückgibt.
...
Der Befehl \textbf{\texttt{input()}} hält das Programm an und wartet, bis die Benutzerin eine Eingabe macht und mit \textbf{Enter} bestätigt. Die Eingabe wird immer als \textbf{String} zurückgegeben. Wenn Ihr Programm einen Zahlenwert benötigt, können Sie die Eingabe mit \textbf{\texttt{int()}} oder \textbf{\texttt{float()}} in den geeigneten Datentypen umwandeln. Bei der Umwandlung kann es zu einem Fehler kommen, z.B. bei der Eingabe \textbf{\texttt{'acht'}}.
...
Bedingungen werden als \textbf{boolesche Ausdrücke} formuliert. Ein boolescher Ausdruck hat den Wert \textbf{\texttt{True}} oder \textbf{\texttt{False}} und wird häufig mit Vergleichsoperatorn (\textbf{\texttt{>}}, \textbf{\texttt{>=}}, \textbf{\texttt{!=}} etc.) und booleschen Operatoren (\textbf{\texttt{and}}, \textbf{\texttt{or}}, \textbf{\texttt{not}}) erzeugt.

\textbf{Wichtig:} Der Vergleichsoperator \textbf{\texttt{==}} prüft auf Gleichheit. Er ist nicht zu verwechseln mit dem Zuweisungsoperator \textbf{\texttt{=}}, der einer Variable einen Wert zuweist.
...
In Python werden Verzweigungen hauptsächlich mit \textbf{\texttt{if}}-Anweisungen realisiert. \textbf{\texttt{if}} prüft eine Bedingung und führt einen Codeblock aus, wenn die Bedingung wahr ist. \textbf{\texttt{elif}} steht für ``else if'' und prüft weitere Bedingungen, nachdem eine vorherige if- oder elif-Bedingung nicht erfüllt wurde. \textbf{\texttt{else}} wird ausgeführt, wenn keine der vorherigen Bedingungen erfüllt wurde. Der \textbf{\texttt{else}}-Zweig ist optional -- eine \textbf{\texttt{if}}-Anweisung kann auch ohne \textbf{\texttt{else}} stehen. if-Anweisungen können innerhalb anderer if-Anweisungen platziert werden (Verschachtelung).
...
\subsection{Endlosschleifen mit \textbf{\texttt{while True}}}

Eine besondere Form der While-Schleife ist die \textbf{Endlosschleife} mit \textbf{\texttt{while True}}. Da die Bedingung \textbf{\texttt{True}} immer wahr ist, läuft die Schleife endlos -- es sei denn, sie wird mit \textbf{\texttt{break}} von innen beendet. Dieses Muster wird oft verwendet, wenn die Abbruchbedingung erst im Schleifenkörper geprüft werden kann.
...
\noindent \textbf{Strategie:} Bei logischen Fehlern hilft \textbf{Print-Debugging}: Fügen Sie \textbf{\texttt{print()}}-Anweisungen ein, um den Wert von Variablen an verschiedenen Stellen im Programm zu überprüfen. So können Sie nachvollziehen, wo das Programm sich anders verhält als erwartet.
